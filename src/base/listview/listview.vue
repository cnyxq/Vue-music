...
const ANCHOR_HEIGHT = 20 // 每个锚点之间的间距
export default {
  name: 'listview',
  created () {
    this.touch = {}
  ...
  methods: {
    ...
    onShortCutTouchMove (el) { // 移动监听
      let firstTouch = el.touches[0]
      this.touch.y2 = firstTouch.pageY
      let delta = (this.touch.y2 - this.touch.y1) / ANCHOR_HEIGHT | 0 // |0相当于向下取整
      let anchorIndex = parseInt(this.touch.anchorIndex) + delta
      this._scrollTo(anchorIndex)
    },
    scroll (pos) { // 子向父传递滚动坐标
      this.scrollY = pos.y
    },
    _scrollTo (index) {
      if (!index) {
    ...
    _calculateHeight () {
      ...
      let height = 0
      this.listHeight.push(height) // 鬼知道为什么要先push一个0进去
  ...
  watch: {
    ...
    scrollY (newY) { // 每当屏幕滚动计算滚动到哪个区间之内
